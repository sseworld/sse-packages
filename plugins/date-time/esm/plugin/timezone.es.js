...
var plugin = function (date, localized_date) {
  ...
  var formatTZ = function (dateObj, arg, timeZone) {
    ...
    var dateObj2 = date.parse(
      new Intl.DateTimeFormat("en-US", options).format(dateObj),
      pattern
    );
    dateObj2.setMilliseconds(dateObj.getMilliseconds());

    dateObj2.getTimezoneOffset = function () {
      return ((utcObj.getTime() - dateObj2.getTime()) / 60000) | 0;
    };
  ...
  var parseTZ = function (dateString, arg, timeZone) {
    ...
    if (
      !timeZone ||
      ~date._parser.find(pattern2, "ZZ").value ||
