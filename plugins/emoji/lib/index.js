...
// src/emojify.ts
var is = __toESM(require("@sindresorhus/is"));

// src/findByName.ts
var import_is = require("@sindresorhus/is");

// src/data.ts
var import_emojilib = __toESM(require("emojilib"));

// src/utils.ts
var import_char_regex = __toESM(require("char-regex"));
...
// src/findByCode.ts
var import_is2 = require("@sindresorhus/is");
var findByCode = (code) => {
  import_is2.assert.string(code);
...
var get = (codeOrName) => {
  import_is3.assert.string(codeOrName);
...
var replace = (input, replacement, { preserveSpaces = false } = {}) => {
  ...
  import_is5.assert.string(input);
  import_is5.assert.function_(replace2);
  import_is5.assert.boolean(preserveSpaces);
...
var search = (keyword) => {
  import_is6.assert.string(keyword);
...
var import_skin_tone = __toESM(require("skin-tone"));
var which = (emoji, { markdown = false } = {}) => {
  import_is7.assert.string(emoji);
  import_is7.assert.boolean(markdown);
