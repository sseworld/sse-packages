...
describe("dedupe", () => {
	...
	it("should make sure subsequent objects can remove/add classes", () => {
		assert.equal(
			dedupe("foo", { foo: false }, { foo: true, bar: true }),
			"foo bar"
		);
	});
	...
	it("should make sure object with falsy value wipe out previous classes", () => {
		assert.equal(
			dedupe("foo foo", 0, null, undefined, false, "b", { foo: false }),
			"b"
		);
		assert.equal(dedupe("foo", "foobar", "bar", { foo: false }), "foobar bar");
		assert.equal(
			dedupe("foo", "foo-bar", "bar", { foo: false }),
			"foo-bar bar"
		);
		assert.equal(
			dedupe("foo", "-moz-foo-bar", "bar", { foo: false }),
			"-moz-foo-bar bar"
		);
	});
